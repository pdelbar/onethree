...
//      $ss = preg_replace("@(&\w+;)@e", 'html_entity_decode("\1")', $ss);
      ...
      $ss = preg_replace_callback("@&#(\d+);@", function ($m) {return chr($m[1]); }, $ss);
      $ss = preg_replace_callback("@(&\w+;)@", function ($m) { return html_entity_decode($m[1]); }, $ss);
